...
test("real Ghidra headless analysis", {
	skip: !ghidraHome || !binary,
	timeout: 20 * 60_000,
}, async () => {
	...
	try {
		...
		const results = inventory.result as Array<{
			action: string;
			result: unknown;
		}>;
		const functions = results[0].result as Array<{
			entry: string;
			name: string;
		}>;
		...
		const inspectedResults = inspected.result as Array<{
			action: string;
			result: unknown;
		}>;
			...
			operations: [
				{
					action: "rename",
					address: entry,
					value: "pi_ghidra_integration_test",
				},
				{
					action: "comment",
					address: entry,
					value: "pi-ghidra integration test",
				},
				{ action: "patch", address: entry, bytes },
			],
		});
		const changed = await runGhidra({
			...base,
			action: "function",
			address: entry,
		});
		assert.equal(
			(changed.result as { name: string }).name,
			"pi_ghidra_integration_test",
		);
		await runGhidra({
			...base,
			action: "batch",
