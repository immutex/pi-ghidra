...
import { ACTIONS, hashFile, resolveGhidraHome, runGhidra } from "../src/runtime.ts";
...
test("explicit Ghidra home and health require no GUI", async () => {
  ...
  try {
    ...
    await writeFile(join(root, "support", process.platform === "win32" ? "analyzeHeadless.bat" : "analyzeHeadless"), "");
    assert.equal(resolveGhidraHome(root), root);
    const result = await runGhidra({ action: "health", ghidraHome: root, cacheDir: join(root, "cache") });
    ...
    await writeFile(join(root, "Ghidra", "application.properties"), "application.version=11.4\n");
...
test("missing binaries fail before Ghidra starts", async () => {
  ...
  try {
    ...
    await writeFile(join(root, "Ghidra", "application.properties"), "application.version=12.1.2\n");
    await writeFile(join(root, "support", process.platform === "win32" ? "analyzeHeadless.bat" : "analyzeHeadless"), "");
    await assert.rejects(runGhidra({ action: "info", binary: "missing.bin", ghidraHome: root }), /Binary not found/);
    const binary = join(root, "sample.bin");
    await writeFile(binary, "abc");
    await assert.rejects(runGhidra({ action: "info", binary, ghidraHome: root, cacheDir: join(root, ".hidden") }), /dot-prefixed/);
    const controller = new AbortController();
    controller.abort();
    await assert.rejects(runGhidra({ action: "info", binary, ghidraHome: root, cacheDir: join(root, "cache") }, process.cwd(), controller.signal), /cancelled/);
  } finally { await rm(root, { recursive: true, force: true }); }
});
